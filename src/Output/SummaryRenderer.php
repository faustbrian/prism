...
final readonly class SummaryRenderer
{
    ...
    private function renderSummary(array $suites): void
    {
        ...
        if ($allPassed) {
            ...
            // Format numbers with consistent width for alignment
            $totalFormatted = sprintf('%5s', number_format($totalTests));
            $passedFormatted = sprintf('%5s', number_format($totalPassed));
            $failedFormatted = sprintf('%5s', number_format($totalFailed));

            $summaryItems = [
                ['label' => 'Total', 'value' => $totalFormatted.' tests', 'color' => 'text-white'],
                ['label' => 'Passed', 'value' => sprintf('%s tests (%.1f%%)', $passedFormatted, $passedPercentage), 'color' => 'text-green'],
                ['label' => 'Failed', 'value' => sprintf('%s tests (%.1f%%)', $failedFormatted, $failedPercentage), 'color' => 'text-red'],
