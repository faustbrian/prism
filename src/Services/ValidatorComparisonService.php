...
final readonly class ValidatorComparisonService
{
        ...
        foreach ($testIds as $testId) {
            ...
            foreach ($validatorNames as $validatorName) {
                ...
                $outcomes[$validatorName] = [
                    ...
                    'expectedValid' => $result->expectedValid,
                ];
            }
     ...
     *
     * Compares the actualValid value across all validators to determine if they
     ...
     *                                                      outcome data including actualValid status
     * @return bool                               True if all validators produced the same
     *                                            actualValid result, false if any disagreement exists
     */
    private function allValidatorsAgree(array $outcomes): bool
    {
        ...
        foreach ($outcomes as $outcome) {
            if ($first === null) {
                $first = $outcome['actualValid'];

                continue;
            }

            if ($outcome['actualValid'] !== $first) {
                return false;
            }
        }
     ...
     *
     * Counts how many validators voted true vs false for actualValid, then
    ...
    private function calculateAgreement(array $outcomes): string
    {
        ...
        foreach ($outcomes as $outcome) {
            $key = $outcome['actualValid'] ? 'true' : 'false';
            ++$validCounts[$key];
        }
