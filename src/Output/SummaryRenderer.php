...
final readonly class SummaryRenderer
{
    ...
    private function renderSummary(array $suites): void
    {
        ...
        // Render Passed
        $dotsNeeded = max(1, $totalWidth - mb_strlen('Passed') - $statsWidth - 1);
        $dots = str_repeat('.', $dotsNeeded);
        render(sprintf(
            <<<'HTML'
                <div class="mx-2">
        ...
        // Render Failed
        $dotsNeeded = max(1, $totalWidth - mb_strlen('Failed') - $statsWidth - 1);
        $dots = str_repeat('.', $dotsNeeded);
        render(sprintf(
            <<<'HTML'
                <div class="mx-2">
        ...
        // Show success banner if all tests passed
        if ($totalFailed === 0) {
            render(<<<'HTML'
